...
class BlogController extends Controller
{
    ...
    public function categories($lang, $slug)
    {
        ...
        $data['featured'] = Blogs::select('id', 'banner', 'banner_alt', 'heading', 'slug', 'short_description')->where('country_id', config('app.country'))->where('lang', app()->getLocale())->where('status',1)->orderBy('id', 'desc')->where('category_id', $data['category']['id'])->first();
