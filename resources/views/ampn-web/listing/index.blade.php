                        ...
                        <div class="col-6 card-product">
                           <div class="text-center position-relative py-1 mb-3 box" style="height:243.031px; width: 170px;">
                              ...
                              <a href="{{route('brand', [$region, $val->slug])}}" >
                                 <!-- img -->
                                 <amp-img src="{{URL::to('/public/storage/retailers/')}}/{{app()->getLocale() == 'ar' ? 'ar/'.$val->ar_logo : $val->logo}}"  layout="responsive" width="190px" height="230.516px" alt="@if(app()->getLocale() == 'ar') {{empty($val->alt_tag_ar) ? $val->name_ar : $val->alt_tag_ar}} @else {{empty($val->alt_tag) ? $val->name : $val->alt_tag}} @endif" class="mb-5 img-fluid"></amp-img>
                              </a>
                           </div>
                        </div>
                     @endforeach
